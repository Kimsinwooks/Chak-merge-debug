...
def patch_torch_load_for_pyannote():
    """
    PyTorch 2.6부터 torch.load 기본값이 weights_only=True로 바뀌면서
    pyannote 3.x checkpoint 로딩이 실패하는 문제를 방지한다.
    pyannote 공식 HF 모델을 신뢰하는 전제에서만 사용한다.
    """
    import torch

    if getattr(torch.load, "_chak_pyannote_patched", False):
        return
    ...
    patched_torch_load._chak_pyannote_patched = True
...
def get_diarization_pipeline():
    ...
    try:
        ...
        pipe = PyannotePipeline.from_pretrained(DIARIZATION_MODEL, use_auth_token=token)
        if pipe is None:
            raise RuntimeError('pyannote pipeline load failed. Check HF_TOKEN and Hugging Face model access.')
...
# =========================================================
# Long audio STT stabilization
# - 30분 이상 긴 음성에서 Whisper/faster-whisper가 후반부 무음/잡음을
#   "네", "응", "음" 등으로 반복 hallucination 하는 문제 방지
# =========================================================
...
def get_wav_duration_sec(wav_path: str) -> float:
    ...
    with wave.open(wav_path, "rb") as wf:
        ...
        return float(total_frames) / float(sr or 1)

def split_wav_to_chunks(wav_path: str, out_dir: str, chunk_sec: int = 30, overlap_sec: int = 1):
    import subprocess
    from pathlib import Path
    ...
    while start < total_sec:
        ...
        chunk_path = out / f"chunk_{idx:05d}_{int(start)}_{int(end)}.wav"
        ...
        if result.returncode == 0 and chunk_path.exists() and chunk_path.stat().st_size > 4096:
            chunks.append({
                "idx": idx,
                "path": str(chunk_path),
                "offset": start,
            ...
            print(f"[STT_CHUNK] ffmpeg chunk failed idx={idx}: {result.stderr[-500:]}")

        idx += 1
        start += max(1, chunk_sec - overlap_sec)

    return chunks

def merge_and_deduplicate_segments(segments):
    segments = sorted(segments, key=lambda x: (x.get("startSec", 0), x.get("endSec", 0)))
    ...
    for seg in segments:
        ...
        start = float(seg.get("startSec", 0) or 0)
        end = float(seg.get("endSec", start) or start)

        compact = normalize_short_text(text)
        last_compact = normalize_short_text(last_text)
        ...
        # 1초 overlap 구간에서 같은 문장이 중복 저장되는 것 방지
        if compact == last_compact and abs(start - last_start) < 2.0:
            continue
...
def filter_repeated_short_segments(segments):
    """
    긴 음성 후반부에서 무음/잡음이 '네', '응', '음' 등으로 반복되는
    hallucination만 제거한다. 단독 짧은 응답 1~2회는 보존한다.
    """
    filtered = []
    recent_short = []
    ...
    for seg in segments:
        ...
        is_short_filler = compact in KOREAN_FILLER_WORDS or len(compact) <= 1

        if is_short_filler and dur <= 2.5:
            recent_short.append((compact, start))
            recent_short = [x for x in recent_short if start - x[1] <= 10]

            # 10초 안에 "네/응/음" 같은 짧은 말이 3개 이상이면 hallucination으로 간주
            if len(recent_short) >= 3:
                print(f"[STT_FILTER] drop repeated short filler: {text} @ {start:.2f}")
                continue
        else:
            recent_short = []
...
def transcribe_audio_chunked_segments(
    model,
    wav_path: str,
    ...
    chunk_dir = Path("data/chunks") / str(session_id) / "stt_chunks"
    ...
    print(f"[STT_CHUNK] chunks={len(chunks)} chunk_sec={chunk_sec} overlap_sec={overlap_sec}")

    all_segments = []

    for ch in chunks:
        print(f"[STT_CHUNK] transcribing {ch['idx'] + 1}/{len(chunks)} offset={ch['offset']:.2f}")
        ...
        for seg in segments_gen:
            ...
            start_sec = float(ch["offset"]) + float(getattr(seg, "start", 0.0) or 0.0)
            end_sec = float(ch["offset"]) + float(getattr(seg, "end", 0.0) or 0.0)
    ...
    all_segments = merge_and_deduplicate_segments(all_segments)
    all_segments = filter_repeated_short_segments(all_segments)

    print(f"[STT_CHUNK] final_segments={len(all_segments)}")
...
def transcribe_audio_with_selected_model(file_path: str, model_name: str = 'medium', language: str = 'ko', diarization_enabled: Optional[bool] = None, speaker_count: Optional[int] = None) -> str:
    ...
    try:
        ...
        asr_segments = transcribe_audio_chunked_segments(
            model=model,
            wav_path=wav_path,
