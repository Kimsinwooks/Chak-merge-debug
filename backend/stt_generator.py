...
def update_session_summary(session_id: int, language, full_text, pretty_text, total_duration, total_silence, silence_events, status="completed", current_silence_run=0, meeting_state="idle"):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("""
...
async def upload_realtime_chunk(
    ...
    try:
        ...
        prev_total = cur.execute("SELECT total_duration, total_silence, silence_events, full_text FROM transcript_sessions WHERE id = ?", (session_id,)).fetchone()
