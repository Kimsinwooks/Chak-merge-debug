...
from session_db import (
    ...
    insert_post_summary,
    read_live_transcript_text_by_session_id,
    read_live_transcript_rows_by_session_id,
)

router = APIRouter()
REALTIME_CHUNK_SECONDS = 15
...
def create_meeting_session(payload: MeetingCreatePayload):
    ...
    channel_id = payload.channel_id or payload.channelId

    c = conn()
    c.execute("""
    INSERT INTO meeting_sessions (
        id, room_name, channel_id, title, meeting_time, keywords, meeting_type,
...
@router.get("/meeting/session/{session_id}/live-transcripts")
def get_live_transcripts(session_id: str):
        ...
        "items": [
            {
                ...
                "startSec": row.get("start_sec") or 0,
                "endSec": row.get("end_sec") or row.get("start_sec") or 0,
                "text": row.get("text") or "",
                "createdAt": row.get("created_at") or "",
                "previewLine": row.get("text") or "",
            }
            for row in rows
        ],
    }
...
async def upload_realtime_chunk(
    ...
    insert_live_transcript(
        ...
        end_sec=offset_sec + REALTIME_CHUNK_SECONDS,
...
def read_session_transcript(session_id: str):
    """
    회의 중 STT 원문은 live_recordings.sqlite3를 기준으로 읽는다.
    meeting_app.sqlite3의 library_items는 과거 데이터 fallback으로만 사용한다.
    """
    ...
    # legacy fallback: 예전에 meeting_app.sqlite3에 저장된 데이터가 있을 때만 사용
    ensure_tables()
    c = conn()
    rows = c.execute(
        """
...
def list_meeting_sessions(room_name: str | None = None):
    ensure_tables()
    c = conn()
...
# =========================
@router.get("/api/realtime-topic-legacy")
def realtime_topic_legacy(session_id: str | None = None, seconds: int = 180):
    ensure_tables()
    ...
    # 15초 chunk 기준: 최근 180초 = 최근 12개 chunk
    # seconds 값이 바뀌어도 chunk 개수를 맞춰 계산
    limit_count = max(1, int(seconds // REALTIME_CHUNK_SECONDS))
    ...
    rows = c.execute(
        ...
        FROM library_items
        WHERE session_id = ?
          AND bucket = 'live_recordings'
        ORDER BY created_at DESC
        LIMIT ?
        """,
        (session_id, limit_count),
    ).fetchall()
    c.close()
...
@router.get("/meeting/session/{session_id}/live-transcripts")
def get_live_transcripts(session_id: str):
    from session_db import read_live_transcript_rows_by_session_id
...
@router.get("/api/realtime-topic")
def realtime_topic(session_id: str | None = None, seconds: int = 180):
    if not session_id:
        return {
            "topic": "세션 없음",
            "currentTopic": "세션 없음",
    ...
    # end_sec 기준 최근 seconds초 sliding window 구성
    max_end = max([
        float(row.get("end_sec") or row.get("start_sec") or 0)
        for row in rows
    ])
    ...
    recent_rows = [
        row for row in rows
        if float(row.get("end_sec") or row.get("start_sec") or 0) >= start_boundary
    ]

    transcript = "\n".join([
        f'{row.get("speaker") or "익명1"}: {(row.get("text") or "").strip()}'
    ...
    if not transcript.strip():
        return {
            "topic": "실시간 STT 대기 중",
            "currentTopic": "실시간 STT 대기 중",
            "summary": "최근 180초 안에 분석할 STT가 없습니다.",
            "sessionId": session_id,
        }
...
""".strip()
    ...
    try:
        prompt = f"{system_prompt}\n\n{user_prompt}\n\nassistant\n"
        topic = call_local_slm(prompt, max_new_tokens=40).strip()
        topic = topic.splitlines()[0].strip() if topic else "실시간 회의 내용 분석 중"
    except Exception as e:
        print(f"[WARN] realtime_topic SLM failed: {e}")
        topic = "실시간 회의 내용 분석 중"
