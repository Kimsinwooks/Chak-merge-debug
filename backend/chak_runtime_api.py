...
def _runtime_transcribe_chunk(file_path: str, offset_sec: float = 0.0) -> str:
    ...
    try:
        ...
        if get_session_noise_filter_enabled(session_id):
            ffmpeg_to_wav_16k_mono_denoise(file_path, wav_path)
        else:
...
class __StableMeetingCreateRequest(_BaseModel):
    title: str = "새 회의"
    meetingTitle: _Optional[str] = None
    meeting_type: str = "general"
    meetingType: _Optional[str] = None
    meeting_time: str = ""
    meetingTime: _Optional[str] = None
    keywords: str = ""
    plan_text: str = ""
    planText: _Optional[str] = None
    realtime_recording_enabled: bool = True
    realtimeRecordingEnabled: _Optional[bool] = None
...
# ============================================================
# FINAL WEB SEARCH OVERRIDE
# - Fixes:
#   1. 프론트가 message 또는 text 중 무엇을 보내도 처리
#   2. 프론트가 useWeb 또는 meta.useWeb 중 무엇을 보내도 처리
#   3. SerpAPI 호출 여부를 백엔드 로그와 debug endpoint로 확인 가능
#   4. 기존 /ai/chat route를 최종 override
# ============================================================
from pydantic import BaseModel as _FinalBaseModel
from typing import Optional as _FinalOptional
import os as ___os
import requests as ___requests


class ___FinalAIChatRequest(_FinalBaseModel):
    message: str = ""
    text: str = ""
    meetingText: str = ""
    mode: str = "general"
    useWeb: bool = False
    sessionId: _FinalOptional[str] = None
    meetingType: str = ""
    meetingTitle: str = ""
    keywords: str = ""
    purpose: str = "chat"
...
def ___runtime_current_web_config():
    return {
        "WEB_SEARCH_PROVIDER": ___os.getenv("WEB_SEARCH_PROVIDER", globals().get("WEB_SEARCH_PROVIDER", "serpapi")).lower(),
        "SERPAPI_API_KEY": ___os.getenv("SERPAPI_API_KEY", globals().get("SERPAPI_API_KEY", "")),
        "SERPAPI_ENGINE": ___os.getenv("SERPAPI_ENGINE", globals().get("SERPAPI_ENGINE", "google")),
        "SERPAPI_GL": ___os.getenv("SERPAPI_GL", globals().get("SERPAPI_GL", "kr")),
        "SERPAPI_HL": ___os.getenv("SERPAPI_HL", globals().get("SERPAPI_HL", "ko")),
        "SERPAPI_LOCATION": ___os.getenv("SERPAPI_LOCATION", globals().get("SERPAPI_LOCATION", "South Korea")),
    }


def ___final_serpapi_search(query: str, use_web: bool) -> str:
    """
    기존 maybe_web_search()가 전역 변수 초기화 타이밍 때문에 꼬일 수 있어서,
    현재 os.environ 기준으로 SerpAPI를 직접 호출하는 최종 검색 함수.
    """
    if not use_web:
        return ""

    cfg = ___runtime_current_web_config()

    if cfg["WEB_SEARCH_PROVIDER"] != "serpapi":
        print(f"[WEB_SEARCH] provider disabled: {cfg['WEB_SEARCH_PROVIDER']}")
        return ""

    if not cfg["SERPAPI_API_KEY"]:
        print("[WEB_SEARCH] SERPAPI_API_KEY missing")
        return ""

    try:
        response = ___requests.get(
            "https://serpapi.com/search.json",
            params={
                "engine": cfg["SERPAPI_ENGINE"],
                "q": query,
                "api_key": cfg["SERPAPI_API_KEY"],
                "hl": cfg["SERPAPI_HL"],
                "gl": cfg["SERPAPI_GL"],
                "location": cfg["SERPAPI_LOCATION"],
                "num": 5,
            },
            timeout=20,
        )
...
def ___final_remove_routes(path: str, methods=None):
    methods = set(methods or [])
    app.router.routes = [
        r for r in app.router.routes
        if not (
...
___final_remove_routes("/ai/chat", {"POST"})


@app.post("/ai/chat")
def final_ai_chat(payload: ___FinalAIChatRequest):
    user_msg = (payload.message or payload.text or "").strip()

    if not user_msg:
        raise HTTPException(status_code=400, detail="질문이 비어 있습니다.")

    # 프론트가 useWeb 또는 meta.useWeb 중 어떤 식으로 보내도 받음
    ...
    if session_id and not meeting_text:
        try:
            meeting_text = _runtime_read_session_transcript(session_id)
        except Exception:
            meeting_text = ""
    ...
    if session_id:
        try:
            rag_text = build_rag_context_text(session_id, user_msg, top_k=5)
        except Exception:
            rag_text = ""

    # 웹검색 문맥
    web_query = f"{meeting_title} {keywords} {user_msg}".strip()
    web_context = ___final_serpapi_search(web_query, use_web)

    print(
        "[AI_CHAT]",
        f"use_web={use_web}",
        f"has_web_context={bool(web_context)}",
...
매우 중요한 규칙:
1. 반드시 한국어로 답변한다.
2. 회의 STT, 회의 자료, RAG 문맥, 웹검색 문맥을 근거로 답한다.
3. useWeb=false이거나 웹검색 결과가 없으면, 외부 최신 정보는 안다고 단정하지 않는다.
4. useWeb=true이고 웹검색 결과가 있으면, 웹검색 결과를 우선 근거로 사용한다.
5. 웹검색 결과로 답할 때는 "웹검색 기준"이라고 짧게 밝혀라.
6. 근거가 부족하면 "현재 업로드된 문서/회의 기록/RAG 검색 결과만으로는 확인할 수 없습니다."라고 말한다.
7. 사실을 지어내지 않는다.
8. 답변은 회의 중 바로 쓸 수 있게 간결하고 구체적으로 작성한다.
""".strip()
...
""".strip()
    ...
    try:
        answer = _runtime_call_ai(system_prompt, user_prompt, model=model_name)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"AI 응답 생성 실패: {str(e)}")
    ...
    return {
        ...
        "ragContextLength": len(rag_text or ""),
    }
...
def final_debug_web_search(q: str = "한림대학교 위치"):
    ...
    try:
        result = ___final_serpapi_search(q, True)
        return {
            "ok": bool(result),
            "query": q,
            "WEB_SEARCH_PROVIDER": cfg["WEB_SEARCH_PROVIDER"],
            "SERPAPI_ENGINE": cfg["SERPAPI_ENGINE"],
            "SERPAPI_GL": cfg["SERPAPI_GL"],
            "SERPAPI_HL": cfg["SERPAPI_HL"],
            "SERPAPI_LOCATION": cfg["SERPAPI_LOCATION"],
            "has_serpapi_key": bool(cfg["SERPAPI_API_KEY"]),
...
# ============================================================
# Realtime STT noise filter patch
# ============================================================
...
def get_session_noise_filter_enabled(session_id: str) -> bool:
    import sqlite3
    ...
    try:
        ...
        conn.row_factory = sqlite3.Row
        row = conn.execute(
            "SELECT noise_filter_enabled FROM meeting_sessions WHERE id=?",
            (session_id,),
        ).fetchone()
        conn.close()
        ...
        return bool(row["noise_filter_enabled"])
    except Exception:
        return True
